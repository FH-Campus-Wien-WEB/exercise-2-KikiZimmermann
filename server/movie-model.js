/* Task 1.1. Add your movie data here 
   and export it so it's available in server.js */


const movies = {
   tt0347149: {
      imdbID: "tt0347149",
      Title: "Howl's Moving Castle",
      Released: "2004-11-20",
      Runtime: 119,
      Genres: ["Animation", "Adventure", "Family"],
      Directors: ["Hayao Miyazaki"],
      Writers: ["Hayao Miyazaki", "Diana Wynne Jones"],
      Actors: ["Chieko Baisho", "Takuya Kimura", "Tatsuya Gashuin"],
      Plot: "When an unconfident young woman is cursed with an old body, her only chance of breaking the spell lies with a self-indulgent wizard and his companions.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMTY1OTg0MjE3MV5BMl5BanBnXkFtZTcwNTUxMTkyMQ@@._V1_SX300.jpg",
      Metascore: 80,
      imdbRating: 8.2
   },

   tt0275847: {
      imdbID: "tt0275847",
      Title: "Lilo & Stitch",
      Released: "2002-06-21",
      Runtime: 85,
      Genres: ["Animation", "Adventure", "Comedy"],
      Directors: ["Dean DeBlois", "Chris Sanders"],
      Writers: ["Chris Sanders"],
      Actors: ["Daveigh Chase", "Chris Sanders"],
      Plot: "A young girl adopts a dog-like alien who helps mend her broken family.",
      Poster: "https://m.media-amazon.com/images/M/MV5BYzRiYTA1YTEtMjYwMi00NGVhLThkOGYtZDBlYjNjZDk2NDEwXkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 74,
      imdbRating: 7.3
   },

   tt0110357: {
      imdbID: "tt0110357",
      Title: "The Lion King",
      Released: "1994-06-24",
      Runtime: 88,
      Genres: ["Animation", "Adventure", "Drama"],
      Directors: ["Roger Allers", "Rob Minkoff"],
      Writers: ["Irene Mecchi", "Jonathan Roberts"],
      Actors: ["Matthew Broderick", "Jeremy Irons", "James Earl Jones"],
      Plot: "Lion prince Simba flees his kingdom after his father is murdered by his uncle.",
      Poster: "https://m.media-amazon.com/images/M/MV5BZGRiZDZhZjItM2M3ZC00Y2IyLTk3Y2MtMWY5YjliNDFkZTJlXkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 88,
      imdbRating: 8.5
   },

   tt0097757: {
      imdbID: "tt0097757",
      Title: "The Little Mermaid",
      Released: "1989-11-17",
      Runtime: 83,
      Genres: ["Animation", "Adventure", "Family"],
      Directors: ["Ron Clements", "John Musker"],
      Writers: ["John Musker", "Ron Clements"],
      Actors: ["Jodi Benson", "Samuel E. Wright", "Rene Auberjonois"],
      Plot: "A mermaid princess makes a deal with a sea witch to become human and win the love of a prince.",
      Poster: "https://m.media-amazon.com/images/M/MV5BNmQ3ODcyZGMtMjNlOS00YzhlLTg0YzAtZGVjNmQ0OTYyNDg0XkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 88,
      imdbRating: 7.6
   },

   tt4729430: {
      imdbID: "tt4729430",
      Title: "Klaus",
      Released: "2019-11-15",
      Runtime: 96,
      Genres: ["Animation", "Adventure", "Comedy"],
      Directors: ["Sergio Pablos", "Carlos Martínez López"],
      Writers: ["Sergio Pablos"],
      Actors: ["Jason Schwartzman", "J.K. Simmons", "Rashida Jones"],
      Plot: "A postman is stationed in a frozen town and discovers a reclusive toymaker named Klaus, leading to an unexpected friendship.",
      Poster: "https://m.media-amazon.com/images/M/MV5BZDA4ODhlMzctNGRmYi00NmViLTkxZGYtZjRkNGQ5YzYwYWVlXkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 65,
      imdbRating: 8.2
   },

   tt2245084: {
      imdbID: "tt2245084",
      Title: "Big Hero 6",
      Released: "2014-11-07",
      Runtime: 102,
      Genres: ["Animation", "Action", "Adventure"],
      Directors: ["Don Hall", "Chris Williams"],
      Writers: ["Jordan Roberts"],
      Actors: ["Ryan Potter", "Scott Adsit"],
      Plot: "A robotics prodigy forms a superhero team with a healthcare robot.",
      Poster: "https://m.media-amazon.com/images/M/MV5BYjQ3YzUyOGQtNjdlYS00YmRhLWIyOWYtMGQ3YjkwMjJiYzRjXkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 74,
      imdbRating: 7.8
   },

   tt0892769: {
      imdbID: "tt0892769",
      Title: "How to Train Your Dragon",
      Released: "2010-03-26",
      Runtime: 98,
      Genres: ["Animation", "Action", "Adventure"],
      Directors: ["Dean DeBlois", "Chris Sanders"],
      Writers: ["Dean DeBlois", "Chris Sanders"],
      Actors: ["Jay Baruchel", "Gerard Butler", "Christopher Mintz-Plasse"],
      Plot: "A young Viking befriends a dragon, challenging his tribe’s traditions.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMjA5NDQyMjc2NF5BMl5BanBnXkFtZTcwMjg5ODcyMw@@._V1_SX300.jpg",
      Metascore: 75,
      imdbRating: 8.1
   },

   tt1646971: {
      imdbID: "tt1646971",
      Title: "How to Train Your Dragon 2",
      Released: "2014-06-13",
      Runtime: 102,
      Genres: ["Animation", "Action", "Adventure"],
      Directors: ["Dean DeBlois"],
      Writers: ["Dean DeBlois"],
      Actors: ["Jay Baruchel", "Cate Blanchett", "Gerard Butler"],
      Plot: "Hiccup and Toothless discover a secret ice cave and face a dangerous new enemy.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMzMwMTAwODczN15BMl5BanBnXkFtZTgwMDk2NDA4MTE@._V1_SX300.jpg",
      Metascore: 77,
      imdbRating: 7.8
   },

   tt2386490: {
      imdbID: "tt2386490",
      Title: "How to Train Your Dragon: The Hidden World",
      Released: "2019-02-22",
      Runtime: 104,
      Genres: ["Animation", "Action", "Adventure"],
      Directors: ["Dean DeBlois"],
      Writers: ["Dean DeBlois"],
      Actors: ["Jay Baruchel", "America Ferrera", "F. Murray Abraham"],
      Plot: "Hiccup seeks a hidden dragon utopia while facing a new threat.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMjIwMDIwNjAyOF5BMl5BanBnXkFtZTgwNDE1MDc2NTM@._V1_SX300.jpg",
      Metascore: 71,
      imdbRating: 7.4
   },

   tt0120855: {
      imdbID: "tt0120855",
      Title: "Tarzan",
      Released: "1999-06-18",
      Runtime: 88,
      Genres: ["Animation", "Adventure", "Family"],
      Directors: ["Chris Buck", "Kevin Lima"],
      Writers: ["Tab Murphy"],
      Actors: ["Tony Goldwyn", "Minnie Driver", "Glenn Close"],
      Plot: "A man raised by gorillas must decide where he truly belongs.",
      Poster: "https://m.media-amazon.com/images/M/MV5BZjM5OWM5NzktMTIzYy00OTk1LWIyYzAtYzBhNWY3NWQ2Y2Y0XkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 79,
      imdbRating: 7.3
   },

   tt0119282: {
      imdbID: "tt0119282",
      Title: "Hercules",
      Released: "1997-06-27",
      Runtime: 93,
      Genres: ["Animation", "Adventure", "Comedy"],
      Directors: ["Ron Clements", "John Musker"],
      Writers: ["Ron Clements", "John Musker"],
      Actors: ["Tate Donovan", "Danny DeVito", "James Woods"],
      Plot: "The son of Zeus must prove himself a hero to regain his place among the gods.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMmUyZGY1ZjItMWYwZS00MjJjLTg1YzAtY2Q5OTA4MmFhYzFjXkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 74,
      imdbRating: 7.3
   },

   tt0053285: {
      imdbID: "tt0053285",
      Title: "Sleeping Beauty",
      Released: "1959-01-29",
      Runtime: 75,
      Genres: ["Animation", "Family", "Fantasy"],
      Directors: ["Clyde Geronimi"],
      Writers: ["Erdman Penner"],
      Actors: ["Mary Costa", "Bill Shirley"],
      Plot: "A princess is cursed to sleep forever until awakened by true love's kiss.",
      Poster: "https://m.media-amazon.com/images/M/MV5BYjVjNjZkNTQtOTRhZi00NTVhLThkMzktYzY4YjIwZWRhZjA0XkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 89,
      imdbRating: 7.2
   },

   tt0441773: {
      imdbID: "tt0441773",
      Title: "Kung Fu Panda",
      Released: "2008-06-06",
      Runtime: 92,
      Genres: ["Animation", "Action", "Comedy"],
      Directors: ["Mark Osborne", "John Stevenson"],
      Writers: ["Jonathan Aibel", "Glenn Berger"],
      Actors: ["Jack Black", "Ian McShane", "Angelina Jolie"],
      Plot: "A clumsy panda is chosen as the Dragon Warrior and must learn kung fu to save his village.",
      Poster: "https://m.media-amazon.com/images/M/MV5BZDU5MDNiMGItYjVmZi00NDUxLTg2OTktNGE0NzNlNzM4NzgyXkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 74,
      imdbRating: 7.6
   },

   tt1302011: {
      imdbID: "tt1302011",
      Title: "Kung Fu Panda 2",
      Released: "2011-05-26",
      Runtime: 90,
      Genres: ["Animation", "Action", "Comedy"],
      Directors: ["Jennifer Yuh Nelson"],
      Writers: ["Jonathan Aibel", "Glenn Berger"],
      Actors: ["Jack Black", "Angelina Jolie", "Dustin Hoffman"],
      Plot: "Po faces a new enemy with a deadly weapon and must confront his past.",
      Poster: "https://m.media-amazon.com/images/M/MV5BYmIxMGYzMTUtZDQzYy00ODc4LWE1YzQtZGMwYTc0YTYyYTE0XkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 67,
      imdbRating: 7.2
   },

   tt2267968: {
      imdbID: "tt2267968",
      Title: "Kung Fu Panda 3",
      Released: "2016-01-29",
      Runtime: 95,
      Genres: ["Animation", "Action", "Comedy"],
      Directors: ["Jennifer Yuh Nelson", "Alessandro Carloni"],
      Writers: ["Jonathan Aibel", "Glenn Berger"],
      Actors: ["Jack Black", "Bryan Cranston", "Dustin Hoffman"],
      Plot: "Po meets his long-lost father and must train a village of pandas to defeat a supernatural villain.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMTUyNzgxNjg2M15BMl5BanBnXkFtZTgwMTY1NDI1NjE@._V1_SX300.jpg",
      Metascore: 66,
      imdbRating: 7.1
   },

   tt2948356: {
      imdbID: "tt2948356",
      Title: "Zootopia",
      Released: "2016-03-04",
      Runtime: 108,
      Genres: ["Animation", "Adventure", "Comedy"],
      Directors: ["Byron Howard", "Rich Moore"],
      Writers: ["Jared Bush"],
      Actors: ["Ginnifer Goodwin", "Jason Bateman"],
      Plot: "A rookie bunny cop and a cynical fox must work together to uncover a conspiracy.",
      Poster: "https://m.media-amazon.com/images/M/MV5BOTMyMjEyNzIzMV5BMl5BanBnXkFtZTgwNzIyNjU0NzE@._V1_SX300.jpg",
      Metascore: 78,
      imdbRating: 8.0
   },

   tt14205554: {
      imdbID: "tt14205554",
      Title: "KPop Demon Hunters",
      Released: "2025-06-20",
      Runtime: 95,
      Genres: ["Animation", "Action", "Comedy"],
      Directors: ["Chris Appelhans", "Maggie Kang"],
      Writers: ["Chris Appelhans", "Maggie Kang", "Danya Jimenez"],
      Actors: ["Arden Cho", "May Hong", "Ji-young Yoo"],
      Plot: "A world-renowned K-Pop girl group balances life in the spotlight with their secret identities as demon hunters.",
      Poster: "https://m.media-amazon.com/images/M/MV5BNTBiYWJlMjQtOTIyMy00NTY4LWFhOWItOWZhNzc3NGMyMjc2XkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 77,
      imdbRating: 7.4
   },

   tt0120762: {
      imdbID: "tt0120762",
      Title: "Mulan",
      Released: "1998-06-19",
      Runtime: 88,
      Genres: ["Animation", "Adventure", "Family"],
      Directors: ["Tony Bancroft", "Barry Cook"],
      Writers: ["Rita Hsiao"],
      Actors: ["Ming-Na Wen", "Eddie Murphy"],
      Plot: "A young woman disguises herself as a warrior to save her father.",
      Poster: "https://m.media-amazon.com/images/M/MV5BYzU0ZTQxZDktOGY0Yy00NjdiLTllNDItMTMzMjI1YjMyYjRhXkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 71,
      imdbRating: 7.7
   },

   tt0165982: {
      imdbID: "tt0165982",
      Title: "Sinbad: Legend of the Seven Seas",
      Released: "2003-07-02",
      Runtime: 88,
      Genres: ["Animation", "Adventure", "Family"],
      Directors: ["Patrick Gilmore", "Tim Johnson"],
      Writers: ["John Logan"],
      Actors: ["Brad Pitt", "Catherine Zeta-Jones", "Joseph Fiennes"],
      Plot: "A sailor named Sinbad is drawn into a quest involving the magical Book of Peace and the goddess Eris.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMTIyMzE1ODI3NF5BMl5BanBnXkFtZTYwMDE3NDc2._V1_SX300.jpg",
      Metascore: 48,
      imdbRating: 6.7
   },

   tt0138749: {
      imdbID: "tt0138749",
      Title: "The Road to El Dorado",
      Released: "2000-03-31",
      Runtime: 89,
      Genres: ["Animation", "Adventure", "Comedy"],
      Directors: ["Bibo Bergeron"],
      Writers: ["Terry Rossio"],
      Actors: ["Kevin Kline", "Kenneth Branagh"],
      Plot: "Two con men discover the legendary city of gold and are mistaken for gods.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMjIyNDBmYTEtYjM3MS00NDhkLWFhNjYtZGYxYzA2OGUyZjQ2XkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 51,
      imdbRating: 6.9
   },

   tt0910970: {
      imdbID: "tt0910970",
      Title: "WALL·E",
      Released: "2008-06-27",
      Runtime: 98,
      Genres: ["Animation", "Adventure", "Family"],
      Directors: ["Andrew Stanton"],
      Writers: ["Andrew Stanton"],
      Actors: ["Ben Burtt", "Elissa Knight"],
      Plot: "A waste-collecting robot embarks on a space journey that will decide the fate of mankind.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMjExMTg5OTU0NF5BMl5BanBnXkFtZTcwMjMxMzMzMw@@._V1_SX300.jpg",
      Metascore: 95,
      imdbRating: 8.4
   },

   tt1217209: {
      imdbID: "tt1217209",
      Title: "Brave",
      Released: "2012-06-22",
      Runtime: 93,
      Genres: ["Animation", "Adventure", "Comedy"],
      Directors: ["Mark Andrews", "Brenda Chapman"],
      Writers: ["Brenda Chapman"],
      Actors: ["Kelly Macdonald", "Billy Connolly"],
      Plot: "A Scottish princess challenges tradition and unleashes chaos.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMzgwODk3ODA1NF5BMl5BanBnXkFtZTcwNjU3NjQ0Nw@@._V1_SX300.jpg",
      Metascore: 69,
      imdbRating: 7.1
   },

   tt0241527: {
      imdbID: "tt0241527",
      Title: "Harry Potter and the Sorcerer's Stone",
      Released: "2001-11-16",
      Runtime: 152,
      Genres: ["Adventure", "Family", "Fantasy"],
      Directors: ["Chris Columbus"],
      Writers: ["J.K. Rowling"],
      Actors: ["Daniel Radcliffe"],
      Plot: "Harry discovers he is a wizard.",
      Poster: "https://m.media-amazon.com/images/M/MV5BNTU1MzgyMDMtMzBlZS00YzczLThmYWEtMjU3YmFlOWEyMjE1XkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 65,
      imdbRating: 7.6
   },

   tt0295297: {
      imdbID: "tt0295297",
      Title: "Harry Potter and the Chamber of Secrets",
      Released: "2002-11-15",
      Runtime: 161,
      Genres: ["Adventure", "Family", "Fantasy"],
      Directors: ["Chris Columbus"],
      Writers: ["J.K. Rowling"],
      Actors: ["Daniel Radcliffe"],
      Plot: "A dark force threatens Hogwarts.",
      Poster: "https://m.media-amazon.com/images/M/MV5BNGJhM2M2MWYtZjIzMC00MDZmLThkY2EtOWViMDhhYjRhMzk4XkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 63,
      imdbRating: 7.4
   },

   tt0304141: {
      imdbID: "tt0304141",
      Title: "Harry Potter and the Prisoner of Azkaban",
      Released: "2004-06-04",
      Runtime: 142,
      Genres: ["Adventure", "Family", "Fantasy"],
      Directors: ["Alfonso Cuarón"],
      Writers: ["J.K. Rowling"],
      Actors: ["Daniel Radcliffe"],
      Plot: "A dangerous prisoner escapes.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMTY4NTIwODg0N15BMl5BanBnXkFtZTcwOTc0MjEzMw@@._V1_SX300.jpg",
      Metascore: 82,
      imdbRating: 7.9
   },

   tt0330373: {
      imdbID: "tt0330373",
      Title: "Harry Potter and the Goblet of Fire",
      Released: "2005-11-18",
      Runtime: 157,
      Genres: ["Adventure", "Family", "Fantasy"],
      Directors: ["Mike Newell"],
      Writers: ["J.K. Rowling"],
      Actors: ["Daniel Radcliffe"],
      Plot: "Harry competes in a deadly tournament.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMTIzNzUzOTk2NV5BMl5BanBnXkFtZTYwNTI4MDg2._V1_SX300.jpg",
      Metascore: 81,
      imdbRating: 7.7
   },

   tt0373889: {
      imdbID: "tt0373889",
      Title: "Harry Potter and the Order of the Phoenix",
      Released: "2007-07-11",
      Runtime: 138,
      Genres: ["Adventure", "Family", "Fantasy"],
      Directors: ["David Yates"],
      Writers: ["J.K. Rowling"],
      Actors: ["Daniel Radcliffe"],
      Plot: "Harry forms a secret student group.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMTIzNzUzOTk2NV5BMl5BanBnXkFtZTYwNTI4MDg2._V1_SX300.jpg",
      Metascore: 71,
      imdbRating: 7.5
   },

   tt0417741: {
      imdbID: "tt0417741",
      Title: "Harry Potter and the Half-Blood Prince",
      Released: "2009-07-15",
      Runtime: 153,
      Genres: ["Adventure", "Family", "Fantasy"],
      Directors: ["David Yates"],
      Writers: ["J.K. Rowling"],
      Actors: ["Daniel Radcliffe"],
      Plot: "Secrets of Voldemort are revealed.",
      Poster: "https://m.media-amazon.com/images/M/MV5BNzU3NDg4NTAyNV5BMl5BanBnXkFtZTcwOTg2ODg1Mg@@._V1_SX300.jpg",
      Metascore: 78,
      imdbRating: 7.6
   },

   tt0926084: {
      imdbID: "tt0926084",
      Title: "Harry Potter and the Deathly Hallows: Part 1",
      Released: "2010-11-19",
      Runtime: 146,
      Genres: ["Adventure", "Family", "Fantasy"],
      Directors: ["David Yates"],
      Writers: ["J.K. Rowling"],
      Actors: ["Daniel Radcliffe"],
      Plot: "The final battle begins.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMTQ2OTE1Mjk0N15BMl5BanBnXkFtZTcwODE3MDAwNA@@._V1_SX300.jpg",
      Metascore: 65,
      imdbRating: 7.7
   },

   tt1201607: {
      imdbID: "tt1201607",
      Title: "Harry Potter and the Deathly Hallows: Part 2",
      Released: "2011-07-15",
      Runtime: 130,
      Genres: ["Adventure", "Family", "Fantasy"],
      Directors: ["David Yates"],
      Writers: ["J.K. Rowling"],
      Actors: ["Daniel Radcliffe"],
      Plot: "The final showdown with Voldemort.",
      Poster: "https://m.media-amazon.com/images/M/MV5BOTA1Mzc2N2ItZWRiNS00MjQzLTlmZDQtMjU0NmY1YWRkMGQ4XkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 85,
      imdbRating: 8.1
   },

   tt0120737: {
      imdbID: "tt0120737",
      Title: "The Lord of the Rings: The Fellowship of the Ring",
      Released: "2001-12-19",
      Runtime: 178,
      Genres: ["Adventure", "Drama", "Fantasy"],
      Directors: ["Peter Jackson"],
      Writers: ["J.R.R. Tolkien", "Fran Walsh", "Philippa Boyens"],
      Actors: ["Elijah Wood", "Ian McKellen", "Orlando Bloom"],
      Plot: "A meek Hobbit from the Shire and eight companions set out on a journey to destroy the powerful One Ring and save Middle-earth from the Dark Lord Sauron.",
      Poster:
         "https://m.media-amazon.com/images/M/MV5BNzIxMDQ2YTctNDY4MC00ZTRhLTk4ODQtMTVlOWY4NTdiYmMwXkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 92,
      imdbRating: 8.9,
   },

   tt0167261: {
      imdbID: "tt0167261",
      Title: "The Lord of the Rings: The Two Towers",
      Released: "2002-12-18",
      Runtime: 179,
      Genres: ["Adventure", "Drama", "Fantasy"],
      Directors: ["Peter Jackson"],
      Writers: ["J.R.R. Tolkien", "Fran Walsh", "Philippa Boyens"],
      Actors: ["Elijah Wood", "Ian McKellen", "Viggo Mortensen"],
      Plot: "While Frodo and Sam edge closer to Mordor with the help of Gollum, the divided fellowship makes a stand against Saruman and his army.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMGQxMDdiOWUtYjc1Ni00YzM1LWE2NjMtZTg3Y2JkMjEzMTJjXkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 87,
      imdbRating: 8.8
   },

   tt0167260: {
      imdbID: "tt0167260",
      Title: "The Lord of the Rings: The Return of the King",
      Released: "2003-12-17",
      Runtime: 201,
      Genres: ["Adventure", "Drama", "Fantasy"],
      Directors: ["Peter Jackson"],
      Writers: ["J.R.R. Tolkien", "Fran Walsh", "Philippa Boyens"],
      Actors: ["Elijah Wood", "Viggo Mortensen", "Ian McKellen"],
      Plot: "Gandalf and Aragorn lead the World of Men against Sauron's army while Frodo and Sam approach Mount Doom to destroy the One Ring.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMTZkMjBjNWMtZGI5OC00MGU0LTk4ZTItODg2NWM3NTVmNWQ4XkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 94,
      imdbRating: 9.0
   },

   tt0903624: {
      imdbID: "tt0903624",
      Title: "The Hobbit: An Unexpected Journey",
      Released: "2012-12-14",
      Runtime: 169,
      Genres: ["Adventure", "Fantasy"],
      Directors: ["Peter Jackson"],
      Writers: ["Fran Walsh", "Philippa Boyens", "Peter Jackson"],
      Actors: ["Martin Freeman", "Ian McKellen", "Richard Armitage"],
      Plot: "A reluctant Hobbit, Bilbo Baggins, sets out on a journey with thirteen dwarves to reclaim their mountain home from the dragon Smaug.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMTcwNTE4MTUxMl5BMl5BanBnXkFtZTcwMDIyODM4OA@@._V1_SX300.jpg",
      Metascore: 58,
      imdbRating: 7.8
   },

   tt1170358: {
      imdbID: "tt1170358",
      Title: "The Hobbit: The Desolation of Smaug",
      Released: "2013-12-13",
      Runtime: 161,
      Genres: ["Adventure", "Fantasy"],
      Directors: ["Peter Jackson"],
      Writers: ["Fran Walsh", "Philippa Boyens", "Peter Jackson"],
      Actors: ["Martin Freeman", "Ian McKellen", "Benedict Cumberbatch"],
      Plot: "The dwarves, along with Bilbo and Gandalf, continue their quest to reclaim Erebor, encountering the dragon Smaug.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMzU0NDY0NDEzNV5BMl5BanBnXkFtZTgwOTIxNDU1MDE@._V1_SX300.jpg",
      Metascore: 66,
      imdbRating: 7.8
   },

   tt2310332: {
      imdbID: "tt2310332",
      Title: "The Hobbit: The Battle of the Five Armies",
      Released: "2014-12-17",
      Runtime: 144,
      Genres: ["Adventure", "Fantasy"],
      Directors: ["Peter Jackson"],
      Writers: ["Fran Walsh", "Philippa Boyens", "Peter Jackson"],
      Actors: ["Martin Freeman", "Ian McKellen", "Richard Armitage"],
      Plot: "Bilbo and the Company are forced to engage in a war against an array of combatants while the Lonely Mountain is reclaimed.",
      Poster: "https://m.media-amazon.com/images/M/MV5BMTYzNDE3OTQ3MF5BMl5BanBnXkFtZTgwODczMTg4MjE@._V1_SX300.jpg",
      Metascore: 59,
      imdbRating: 7.4
   },

   tt9999999: {
      imdbID: "tt9999999",
      Title: "Genre Test Movie",
      Released: "2026-04-07",
      Runtime: 120,
      Genres: ["Action", "Adventure", "Animation", "Biography", "Comedy", "Crime", "Documentary", "Drama", "Family", "Fantasy", "Film Noir", "History", "Horror", "Music", "Musical", "Mystery", "Romance", "Sci-Fi", "Short Film", "Sport", "Superhero", "Thriller", "War", "Western"],
      Directors: ["Test Director"],
      Writers: ["Test Writer"],
      Actors: ["Actor One", "Actor Two", "Actor Three"],
      Plot: "This is a test movie used to check whether all genre colors and form fields work correctly.",
      Poster: "https://m.media-amazon.com/images/M/MV5BZjA4YjQ4Y2UtY2Q4MC00Njk5LWExOTAtOWQyYjM0MjQ0YzVlXkEyXkFqcGc@._V1_SX300.jpg",
      Metascore: 100,
      imdbRating: 10.0
   }
}

module.exports = movies